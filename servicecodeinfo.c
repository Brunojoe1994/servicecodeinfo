...
servicecodeinfo is a small C utility that analyzes DCCP service codes (RFC 4340) and reports their ASCII representation and allocation policy.

Overview

This tool accepts service codes either as numeric values or as short ASCII strings (1–4 characters). It converts between representations and classifies each code based on IANA allocation rules.

Features
Supports decimal and hexadecimal input
Accepts ASCII service names via -n
Outputs allocation policy (Reserved, Private Use, FCFS, Specification Required)
...
servicecodeinfo NUMBER [NUMBER...]

servicecodeinfo -n NAME [NAME...]
...
Public domain (as declared by the original author).

Notes

DCCP is not widely used in modern systems, so this tool is mainly useful for networking research, protocol experimentation, or legacy environments.
